...
abstract class BaseRepository implements RepositoryInterface, RepositoryCriteriaInterface
{
    ...
    {
        if (method_exists($this, 'ind_situ')) {        
            $this->orderBy('ind_situ');
        }
     ...
     * @return void
     */
    public function where(array $where, array $orWhere = [])
    {
        if ($where) {
            $this->applyConditions($where);
        }
        if ($orWhere) {
            $this->applyConditionsOr($orWhere);
        }
        return $this;
    }
     ...
     * @return mixed
     */
    public function findWhere(array $where, $columns = ['*'], $orWhere = [])
    {
        $this->applyCriteria();
        $this->applyScope();
     ...
     * @return mixed
     */
    private function applyConditionsOr(array $orWhere)
    {
        $this->model->where(function ($query) use ($orWhere) {
    ...
    /**
     * Update Mass in repository by where
     *
     * @throws ValidatorException
     *
     * @param array $attributes
     ...
     * @return mixed
     */
    public function updateMass(array $attributes, array $where)
    {
        $regUpdate = false;
        foreach ($where as $kWhere => $vWhere) {
            if ($regUpdate) {
                $regUpdate->where($kWhere, $vWhere);
                continue;
            } else {
                $regUpdate = $this->model::where($kWhere, $vWhere);
            }
        }
        if ($regUpdate) {
            $regUpdate->update($attributes);
        }
        $this->resetModel();
        return $this->parserResult($regUpdate);
    }
     ...
     * @return $this
     */
    public function orderByNullLast($column, $direction = 'asc')
    {
        $this->model = $this->model->orderByNullLast($column, $direction);
     ...
     * @return $this
     */
    public function orderByFixed($column, $orderValue)
    {
        $this->model = $this->model->orderByFixed($column, $orderValue);
    ...
    protected function applyConditions(array $where)
    {
        foreach ($where as $field => $value) {
            if (is_array($value)) {
                list($field, $condition, $val) = $value;
                if ($value === null) {
                    $this->model = (!empty($condition) && $condition == '<>') ? $this->model->whereNotNull($field) : $this->model->whereNull($field);
                } else {

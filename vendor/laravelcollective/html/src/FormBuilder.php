...
class FormBuilder
{
    ...
    public function label($name, $value = null, $options = [], $escape_html = true)
    {
        ...
        if (isset($this->model)) {
            // verifica e add required
            $this->labelRequired($name, $options);
            
            // verifica e add attributeLabel 
            if (!$value && property_exists($this->model, 'attributeLabel')) {
                $value = $this->model->getAttributeLabel($name);
            }
        }
     ...
     * @return void
     */
    protected function labelRequired(string $name, array &$options)
    {
        // para nao add o *: crie a key 'skip' => 'required' em $options
        if (!$this->skipRequired($options)) {
            if ($this->model->getAttributeRequired($name)) {
                $options['class'] = empty($options['class']) ? 'required' : (is_array($options['class']) ? implode(' ', $options['class']) . ' required' : $options['class'] . ' required');
            }
        }
    }

    /**
     * Add required true in $options.
     *
     * @param  string     $name
     * @param  array      &$options
     *
     * @return void
     */
    protected function inputRequired(string $name, array &$options)
    {
        // para nao add o required no input: crie a key 'skip' => 'required' em $options
        if (!$this->skipRequired($options)) {
            if (isset($this->model)) {
                if ($this->model->getAttributeRequired($name) && !array_key_exists('readonly', $options)) {
    ...
    /**
     * Check skip required in $options.
     *
     * @param  array      $options
     *
     * @return bool
     */
    protected function skipRequired(array $options)
    {
        return (!empty($options['skip']) && !empty(in_array('required', explode(' ', $options['skip']))));
    }
    ...
    public function input($type, $name, $value = null, $options = [])
    {
        ...
        // verifica e add required no input
        if ($name) {
            $this->inputRequired($name, $options);
        }

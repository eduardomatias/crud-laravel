...
class ViewGenerator extends BaseGenerator
{
    ...
    {
        $templateData = get_template('scaffold.views.search_table', $this->templateType);

        $templateData = fill_template($this->commandData->dynamicVars, $templateData);
        
        $templateData = str_replace('$SEARCHFIELDS$', $this->generateSearchFields(), $templateData);
    ...
    private function generateSearchFields()
    {
        $searchFields = [];

        foreach ($this->commandData->fields as $field) {
            if (!$field->isSearchable || $field->htmlInput != 'text') {
                continue;
            }
            $searchFields[] = $field->name . ':ilike';
        }
        ...
        foreach ($this->commandData->fields as $field) {
            ...
            if (!empty($fieldTemplate)) {
                ...
                if (str_contains($field->validations, 'required')) {
                    $fieldTemplate = str_replace('$FIELD_CLASS_REQUIRED$', 'required', $fieldTemplate);
                } else {
                    $fieldTemplate = str_replace('$FIELD_CLASS_REQUIRED$', '', $fieldTemplate);
                }
